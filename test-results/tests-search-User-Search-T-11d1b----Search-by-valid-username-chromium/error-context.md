...
Locator: locator('.oxd-table-body .oxd-table-row').first().locator('.oxd-table-cell').nth(3)
Expected string: "manda123 user"
Received string: "manda user"
Call log:
  - expect.toHaveText with timeout 5000ms
  - waiting for locator('.oxd-table-body .oxd-table-row').first().locator('.oxd-table-cell').nth(3)
    9 × locator resolved to <div role="cell" class="oxd-table-cell oxd-padding-cell">…</div>
      - unexpected value "manda user"

    at /Users/Playwright/e2e-playwright-framework/tests/search.test.ts:29:30
```
...
> 29 |   await expect(cells.nth(3)).toHaveText('manda123 user');       // Employee Name
  ...
  48 |     
  49 |     // Locate the exact toast message using XPath
  50 |     const toast = page.locator('xpath=//*[@id="oxd-toaster_1"]/div/div[2]/div');
  51 |
  52 |     // Wait for the toast to appear and validate the text
  53 |     await expect(toast).toHaveText('No Records Found', { timeout: 5000 });
  54 |   });
  55 |
  56 |   test('TC_SEARCH_004 - Search with empty filters', async ({ page }) => {
  57 |     const admin = new AdminPage(page);
  58 |     await admin.goto();
  59 |     await admin.clearSearch();
  60 |     await expect(await admin.getResultsText().first()).not.toContainText('No Records Found');
  61 |   });
  62 | });
  63 |
```