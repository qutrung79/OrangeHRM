...
Locator: locator('.oxd-table-row.oxd-table-row--empty')
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for locator('.oxd-table-row.oxd-table-row--empty')

    at /Users/Playwright/e2e-playwright-framework/tests/search.test.ts:50:28
```
...
```ts
  ...
  49 |     const emptyRow = page.locator('.oxd-table-row.oxd-table-row--empty');
> 50 |     await expect(emptyRow).toBeVisible();
     |                            ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
  51 |     await expect(emptyRow).toContainText('No Records Found');
  52 |   });
  53 |
  54 |   test('TC_SEARCH_004 - Search with empty filters', async ({ page }) => {
  55 |     const admin = new AdminPage(page);
  56 |     await admin.goto();
  57 |     await admin.clearSearch();
  58 |     await expect(await admin.getResultsText().first()).not.toContainText('No Records Found');
  59 |   });
  60 | });
  61 |
```