...
```
Error: expect.toContainText: Error: strict mode violation: locator('.oxd-table-body .oxd-table-cell') resolved to 6 elements:
    1) <div role="cell" class="oxd-table-cell oxd-padding-cell">…</div> aka getByRole('cell', { name: '' })
    ...
    4) <div role="cell" class="oxd-table-cell oxd-padding-cell">…</div> aka getByRole('cell', { name: 'manda123 user' })
    5) <div role="cell" class="oxd-table-cell oxd-padding-cell">…</div> aka getByRole('cell', { name: 'Enabled' })
    6) <div role="cell" class="oxd-table-cell oxd-padding-cell">…</div> aka getByRole('cell', { name: ' ' })

Call log:
  - expect.toContainText with timeout 5000ms
  - waiting for locator('.oxd-table-body .oxd-table-cell')

    at /Users/Playwright/e2e-playwright-framework/tests/search.test.ts:50:32
```
...
```ts
  ...
  49 |     const noRecordCell = page.locator('.oxd-table-body .oxd-table-cell');
> 50 |     await expect(noRecordCell).toContainText('No Records Found');
     |                                ^ Error: expect.toContainText: Error: strict mode violation: locator('.oxd-table-body .oxd-table-cell') resolved to 6 elements:
  51 |   });
  52 |
