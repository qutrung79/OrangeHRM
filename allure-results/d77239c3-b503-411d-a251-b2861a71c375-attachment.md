...
- Name: Login Tests >> TC_LOGIN_004 - Login with blank username
- Location: /Users/Playwright/e2e-playwright-framework/tests/login.test.ts:26:7
...
Call log:
  ...
  - waiting for locator('.oxd-alert-content-text')
    ...
    at /Users/Playwright/e2e-playwright-framework/tests/login.test.ts:30:49
```
...
> 30 |     await expect(await login.getErrorMessage()).toContainText('Required');
     |                                                 ^ Error: Timed out 5000ms waiting for expect(locator).toContainText(expected)
  31 |   });
  32 |
  33 |   test('TC_LOGIN_005 - Login with blank password', async ({ page }) => {
  34 |     const login = new LoginPage(page);
  35 |     await login.goto();
  36 |     await login.login('Admin', '');
  37 |     await expect(await login.getErrorMessage()).toContainText('Required');
  38 |   });
  39 | });
  40 |
```